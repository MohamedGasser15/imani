...
class SettingsProvider extends ChangeNotifier {
  ...
  Future<void> _loadSettings() async {
    ...
    _locale = Locale(prefs.getString('languageCode') ?? 'ar');
  ...
  Future<void> setTimeFormat(TimeFormat format) async {
    ...
    _timeFormat = format;
    await SharedPreferences.getInstance().then((prefs) {
      prefs.setInt('timeFormat', format.index);
    });
    notifyListeners();
  }
  ...
  Future<void> setAdhanEnabled(bool enabled) async {
    ...
    await SharedPreferences.getInstance().then((prefs) {
      prefs.setBool('isAdhanEnabled', enabled);
    });
    notifyListeners();
  }
  ...
  Future<void> setQuranFontSize(double size) async {
    _quranFontSize = size;
    await SharedPreferences.getInstance().then((prefs) {
      prefs.setDouble('quranFontSize', size);
    });
    notifyListeners();
  }
}